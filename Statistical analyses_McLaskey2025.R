...
# tiff("TFA timeseries 20250204.tiff", width=100, height=70, units="mm", res=500)
# TFA.plot
# dev.off()
...
bulk.POM$DHA.EPA
bulk.POM$Bacteria_no15 <- (bulk.POM$C17.0_PERCENT + 
                             bulk.POM$iso.15.0_PERCENT + 
                             bulk.POM$iso.17.0_PERCENT + 
                             bulk.POM$ant.15.0_PERCENT)

bulk.POM$Species <- "Bulk POM"


zoopsprint.bulk <- full_join(zoopData.sm, bulk.POM)

zoopsprint.bulk.means <- zoopsprint.bulk %>% dplyr::select(Species, Month, SumFA_mg.g,
                                                   DHA.EPA, percent.SFA, percent.PUFA, percent.MUFA,
                                                   C18.3andC18.4, C16PUFA, Bacteria_no15, Copes.C20C.1C22.1,
                                                   Diatom.Flag, Ratio16.1, C16.3n.4_PERCENT, 
                                                   Date) %>% 
...
zoopsprint.bulk.wMeans <- zoopsprint.bulk.means %>% dplyr::select(-Date) %>% 
  full_join(.,zoopsprint.bulk, by = join_by(Species,Month))
...
pDHA.EPA <- ggplot(zoopsprint.bulk.wMeans, aes(x=Date, y=mean.DHA.EPA, color=Species, fill=Species)) + 
  ...
  scale_linetype_manual(values = c("dashed", "solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid", "solid")) + 
  scale_color_manual(values = POMclrs) + 
  scale_fill_manual(values =  POMclrs)  +
  labs(x="", y="DHA:EPA",
       color="Species" , fill="Species") 
pDHA.EPA
# tiff("DHA.EPA 20250204.tiff", width=110, height=100, units="mm", res=500)
# pDHA.EPA
# dev.off() 
...
pSFA
# tiff("SFA 20250204.tiff", width=110, height=100, units="mm", res=500)
# pSFA
# dev.off()
...
pPUFA
# tiff("PUFA 20250204.tiff", width=110, height=100, units="mm", res=500)
# pPUFA
# dev.off()
...
pBAFA
# tiff("Bacterial FA 20250204.tiff", width=110, height=100, units="mm", res=500)
# pBAFA
# dev.off()
...
pC16PUFA
# tiff("C16PUFA 20250204.tiff", width=110, height=100, units="mm", res=500)
# pC16PUFA
# dev.off()
...
pC18PUFA
# tiff("C18PUFA 20250204.tiff", width=110, height=100, units="mm", res=500)
# pC18PUFA
# dev.off()
...
# tiff("Chl a timeseries 20240701.tiff", width=100, height=70, units="mm", res=500)
# pChl.allconcs
# dev.off() 
...
# tiff("Chl a lengend 20240629.tiff", width=100, height=60, units="mm", res=500)
# pChl.legend
# dev.off() 



# Overlay of Bulk POM Total FA concentrations 

POM.sizes$Species <- POM.sizes$Pore.Size

zsAllw.POM <- full_join(zoopData.sm, POM.sizes)

ggp_transparent1<- ggplot(bulk.POM, aes(x=as.factor(Month), y=SumFA_ug.L, fill=Type, color=Type)) + 
  scale_fill_manual(values = "#CC9933") + 
  scale_color_manual(values = "#996600") + 
  geom_boxplot() + 
  labs(x="", color="Species" , fill="Species",
       y=expression(paste("POM FA (",mu,"g L"^-1*")"))) + 
...
#        filename = "ggp_transparent5.png",
#        bg = "transparent",
#        width=60, height=70, unit="mm", dpi=500)
...
SI.all <- read.csv("processed_data/full_zoopsprint SI 20250317.csv")
...
SI.POM <- SI.all %>% filter(Species=="POM") %>% 
  filter(Pore.Size != "bulk") %>%   select(Date, Pore.Size, 
...
sizes.N <- ggplot(SI.POM, aes(x=as.factor(Month), y=delta15n, fill=Pore.Size)) + 
  geom_boxplot(width = 0.5) + labs(x="") + 
  scale_y_continuous(limits=c(2.5, 11.5)) +
  scale_x_discrete(labels = c("May", "Jul", "Sep", "Nov", "Jan")) +
  scale_fill_manual(values=c("#c2e699","#78c679", "#006837")) + 
  labs(y=expression(paste(delta^{15}, "N")), x="")+ 
  theme_classic() + theme_bw() + 
  theme(legend.position = "none",
...
# png("202502232 Figure 2.png", width=180, height=130, units="mm", res=300)
# prow
# dev.off()
...
POM.sizes$Pore.Size <- as.factor(POM.sizes$Pore.Size)

POM.sizes.wSI <- right_join(SI.POM, POM.sizes)


isotopes <- POM.sizes.wSI %>% select(delta13c, delta15n)
colnames(isotopes)[c(1:2)] <- c("del13C", "del15N")
...
NMDS.bc<-data.frame(NMDS1.bc=eco.nmds.bc$points[,1],NMDS2.bc=eco.nmds.bc$points[,2], month=Months, size=sizes)
...
POM.NMDS <- ggplot(NMDS.bc, aes(NMDS1.bc, NMDS2.bc))+ theme_bw() + 
  ...
  geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
               data = en_coord_cont, lwd=0.6, colour = "black", alpha = 0.4) +
  geom_point(stat = "identity", aes(color= Months, shape=size, fill=Months), size=1, stroke = 0.8) +
  scale_shape_manual(values = c(2,21,8))  +
  
  scale_color_manual(values =c("#053061",
                                        # "#66C2A5",
                                        "#A6D96A",  "#FDAE61", 
                                        "#D53E4F",  "#67001F"), name="Month", guide="legend") +
  ...
  labs(x="NMDS1",
       y="NMDS2") + 
  annotate("text", x=en_coord_cont[c(1),1]*1.1, y=en_coord_cont[c(1),2]*1.1, 
...
# tiff("POMFA NMDS w SI 20240712.tiff", width=3.5, height=3, units="in", res=500)
# POM.NMDS
# dev.off()
...
# Make reduced data matrix
Q20.matrix <- zoopData.sm %>%  dplyr::select(C14.0_PERCENT:iso.17.0_PERCENT)
...
# write.csv(fullsummary, "Taxa FA means 2024-01-22.csv")
...
# png("ZS NMDS 20240604.png", width=120, height=120, units="mm", res=300)
# NMDS1
# dev.off()
...
allSIdata.sm <- Zoop.SI %>% select(Species, Month, delta13c, delta15n)
colnames(allSIdata.sm)[4] <- c("delta15n")

taxa.names <- as.factor(allSIdata.sm$Species)
Months <- as.factor(allSIdata.sm$Month)
allSIdata.sm$Species <- as.factor(allSIdata.sm$Species)
...
#dataframe for plotting NMDS
NMDS.bc.SI<-data.frame(NMDS1.bc=allSIdata.sm[,3],NMDS2.bc=allSIdata.sm[,4], group=taxa.names, Month=allSIdata.sm[,1])

plot.new()
ord.bc.SI <-ordiellipse(allSIdata.sm[,3:4], taxa.names, display="sites",kind="sd", conf = 0.95, label=T)
...
pSIbiplot <- ggplot(NMDS.bc.SI, aes(NMDS1.bc, NMDS2.bc))+
  ...
  geom_path(data=df_ell.bc.SI, aes(x=delta13c, y=delta15n, colour=group), size=1.5) +
  scale_fill_manual(values=clrs) +
  labs(x=expression(paste(delta^{13}, "C")), 
       y=expression(paste(delta^{15}, "N")))+ 
  scale_color_manual(values=clrs) + theme_bw() + 
...
pSIbiplot
 ...
 # png("ZS SI biplot 20240223.png", width=100, height=120, units="mm", res=300)
 # pSIbiplot
 # dev.off()
...
# png("20250218 Trophic position plot.png", width=240, height=90, units="mm", res=300)
# prow
# dev.off()
...
# png("Dendrogram 20241002.png", width=200, height=500, units="mm", res=300)
par(mar=c(0,1,0,6))
dend %>%
  set("labels_cex", 0.8 )%>% set("branches_k_color", k=6, value = c("#CD0BBC", "#2297E6","#61D04F", "black", "#28E2E5", "#DF536B"),alpha=0.30) %>%
  plot(horiz=T)
# dev.off()
...
# png("NMDS w projected vars 20250223.png", width=112, height=112, units="mm", res=300)
# gg
# dev.off()
